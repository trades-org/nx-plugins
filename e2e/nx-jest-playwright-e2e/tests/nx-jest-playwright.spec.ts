...
describe('nx-jest-playwright e2e', () => {
  beforeAll(() => {
    ensureComplexNxProject(
      ['@trades-org/nx-jest-playwright', 'dist/packages/nx-jest-playwright'],
  ...
  it('should create nx-jest-playwright', async () => {
    ...
    await runNxCommandAsync(`generate @trades-org/nx-jest-playwright:project ${plugin}`);
  ...
  describe('--directory', () => {
    it('should create src in the specified directory', async () => {
      ...
      await runNxCommandAsync(
        `generate @trades-org/nx-jest-playwright:project ${plugin} --directory subdir`,
      );
  ...
  describe('--tags', () => {
    it('should add tags to project.json', async () => {
      const plugin = uniq('nx-jest-playwright');
      await runNxCommandAsync(
        `generate @trades-org/nx-jest-playwright:project ${plugin} --tags e2etag,e2ePackage`,
      );
