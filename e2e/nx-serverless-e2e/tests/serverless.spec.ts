...
describe('serverless e2e', () => {
  ...
  describe('@trades-org/nx-serverless/plugin', () => {
    it('should create serverless', async () => {
      ...
      await runNxCommandAsync(
        `generate @trades-org/nx-serverless:application ${plugin} --plugin @trades-org/nx-serverless/plugin`,
      );
  ...
  describe('--directory and --tags', () => {
    it('should create src in the specified directory with tags', async () => {
      ...
      await runNxCommandAsync(
        `generate  @trades-org/nx-serverless:application ${plugin} --directory subdir --tags e2etag,e2ePackage`,
      );
